...
class MLBacktester():
    ...
    def prepare_features(self, start, end):
        ...
        self.data_subset["dir"] = np.where(self.data_subset["returns"] > 0, 1, -1)
        self.data_subset["sma"] = self.data_subset[self.symbol].rolling(window).mean() - self.data_subset[self.symbol].rolling(50).mean()
        self.data_subset["boll"] = (self.data_subset[self.symbol] - self.data_subset[self.symbol].rolling(window).mean()) / self.data_subset[self.symbol].rolling(window).std()
        self.data_subset["min"] = self.data_subset[self.symbol].rolling(window).min() / self.data_subset[self.symbol] - 1
        self.data_subset["max"] = self.data_subset[self.symbol].rolling(window).max() / self.data_subset[self.symbol] - 1
